/**
 * HIS Layer Initializer
 * 将 HIS 工具添加到已运行的 v6 系统
 */
...
// 注册 HIS 工具到 global.claw2eeTools
function registerHISTools() {
  if (!global.claw2eeTools) {
    console.log('[HIS] global.claw2eeTools not found');
    return false;
  }
  ...
  const hisTools = {
    ...
    his_health: {
      ...
      handler: function(args, callback) { callback(null, HIS.L5.check()); }
    },
    his_veto: {
      description: '[L5] 最高权限否决检查',
      inputSchema: { type: 'object', properties: { output: { type: 'object' } }, required: ['output'] },
      handler: function(args, callback) { callback(null, HIS.L5.veto(args.output)); }
    },
    his_circuit_breaker: {
    ...
    his_audit: {
      ...
      handler: function(args, callback) { callback(null, HIS.L4.audit(args.l1 || {}, args.l2 || {}, args.l3 || {})); }
    },
    his_blindspot: {
      description: '[L4] 标记盲区',
    ...
    his_get_blindspots: {
      description: '[L4] 获取盲区',
      inputSchema: { type: 'object' },
      handler: function(args, callback) { callback(null, { blindspots: HIS.L4.getBlindspots() }); }
    },
    ...
    his_align: {
      ...
      handler: function(args, callback) { callback(null, HIS.L3.align(args.plan, args.goal)); }
    },
    his_risk_assess: {
      description: '[L3] 风险评估',
      inputSchema: { type: 'object', properties: { plan: { type: 'object' } }, required: ['plan'] },
      handler: function(args, callback) { callback(null, HIS.L3.risk(args.plan)); }
    },
    ...
    his_understand: {
      ...
      handler: function(args, callback) { callback(null, HIS.L2.understand(args.query)); }
    },
    his_predict: {
      description: '[L2] 状态预测',
      inputSchema: { type: 'object', properties: { state: { type: 'object' } }, required: ['state'] },
      handler: function(args, callback) { callback(null, HIS.L2.predict(args.state)); }
    },
    his_generate_plan: {
      description: '[L2] 生成候选方案',
      inputSchema: { type: 'object', properties: { goal: { type: 'string' } }, required: ['goal'] },
      handler: function(args, callback) { callback(null, { plans: HIS.L2.genPlan(args.goal) }); }
    },
    ...
    his_prove: {
      ...
      handler: function(args, callback) { callback(null, HIS.L1.prove(args.statement)); }
    },
    his_compute: {
      description: '[L1] 数学计算',
      inputSchema: { type: 'object', properties: { expr: { type: 'string' } }, required: ['expr'] },
      handler: function(args, callback) { callback(null, HIS.L1.compute(args.expr)); }
    },
    his_check_consistency: {
      ...
      handler: function(args, callback) { callback(null, HIS.L1.consistency(args.statements)); }
    },
    ...
    his_perceive: {
      ...
      handler: function(args, callback) { callback(null, HIS.L0.perceive(args.input)); }
    },
    his_read_memory: {
      description: '[L0] 读取短期记忆',
      inputSchema: { type: 'object' },
      handler: function(args, callback) { callback(null, { memory: HIS.L0.readMem(), count: HIS.L0.readMem().length }); }
    },
    his_write_memory: {
    ...
    his_status: {
      ...
      handler: function(args, callback) {
        ...
        callback(null, {
          ...
          total_tools: Object.keys(global.claw2eeTools).length + 17,
          timestamp: Date.now()
        });
      }
    }
  ...
  // 添加到 global.claw2eeTools
  let count = 0;
  for (const [name, tool] of Object.entries(hisTools)) {
    if (!global.claw2eeTools[name]) {
      global.claw2eeTools[name] = tool;
      count++;
    }
  }
  ...
  console.log('[HIS] Registered ' + count + ' HIS tools to v6');
  return true;
}

// 等待 gateway 启动后注册
setTimeout(function() {
  console.log('[HIS] Initializing HIS layer...');
  registerHISTools();
  console.log('[HIS] HIS layer ready');
}, 5000);
